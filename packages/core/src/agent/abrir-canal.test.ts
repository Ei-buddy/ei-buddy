...
describe('o telefone do provedor vira o do cadastro', () => {
  ...
  it('nao tira o 55 de um numero que nao tem DDI', () => {
    ...
    expect(normalizarTelefoneDoCanal('5599998888')).toBe('55999998888')
    expect(normalizarTelefoneDoCanal('5599998888').startsWith('55')).toBe(true)
  })
...
describe('nono digito movel — NR-046 US3', () => {
  const REGRA_NONO =
    'O 9 entra depois do DDD so no nacional de 10 digitos cujo primeiro digito do assinante e 6, 7, 8 ou 9.'

  it(`${REGRA_NONO} — chaves de 554188888888`, () => {
    expect(chavesConsultaTelefoneDoCanal('554188888888')).toEqual(['41988888888', '4188888888'])
  })

  it(`${REGRA_NONO} — chaves de 4188888888`, () => {
    expect(chavesConsultaTelefoneDoCanal('4188888888')).toEqual(['41988888888', '4188888888'])
  })

  it('mascara +55 (41) 98888-8888 canonicaliza para 41988888888', () => {
    expect(normalizarTelefoneDoCanal('+55 (41) 98888-8888')).toBe('41988888888')
  })

  it('fixo 4133334444 nao ganha segunda chave', () => {
    expect(normalizarTelefoneDoCanal('4133334444')).toBe('4133334444')
    expect(chavesConsultaTelefoneDoCanal('4133334444')).toEqual(['4133334444'])
  })

  it('DDD 55 nao e DDI — movel canonical 55999998888 e legado 5599998888', () => {
    expect(chavesConsultaTelefoneDoCanal('5599998888')).toEqual(['55999998888', '5599998888'])
    expect(normalizarTelefoneDoCanal('5599998888').startsWith('55')).toBe(true)
  })

  it('554188888888 acha cadastro 41988888888 na primeira chave', async () => {
    const d = diretorio({ '41988888888': { companyId: EMPRESA, userId: DONA } })

    const r = await abrir(d.peers, '554188888888')

    expect(d.consultados).toEqual(['41988888888'])
    expect(r.status).toBe('autorizado')
  })

  it('4188888888 tenta 41988888888 antes do legado', async () => {
    const consultados: string[] = []
    const peers: PeerDirectory = {
      porTelefone: async (phone) => {
        consultados.push(phone)
        if (phone === '41988888888') {
          return { companyId: EMPRESA, userId: DONA }
        }
        return undefined
      },
    }

    const r = await abrir(peers, '4188888888')

    expect(consultados).toEqual(['41988888888'])
    expect(r.status).toBe('autorizado')
  })

  it('4188888888 cai no legado se so o de 10 digitos existir', async () => {
    const d = diretorio({ '4188888888': { companyId: EMPRESA, userId: DONA } })

    const r = await abrir(d.peers, '4188888888')

    expect(d.consultados).toEqual(['41988888888', '4188888888'])
    expect(r.status).toBe('autorizado')
  })
})
